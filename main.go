...
func md2html(input io.Reader) string {
	...
	for scanner.Scan() {
		...
		if line[0] == '#' {
			...
			switch count {
			case 1:
				line = h1Reg.ReplaceAll(line, []byte(`<h1>$1</h1>`))
			case 2:
				line = h2Reg.ReplaceAll(line, []byte(`<h2>$1</h2>`))
			case 3:
				line = h3Reg.ReplaceAll(line, []byte(`<h3>$1</h3>`))
			case 4:
				line = h4Reg.ReplaceAll(line, []byte(`<h4>$1</h4>`))
			case 5:
				line = h5Reg.ReplaceAll(line, []byte(`<h5>$1</h5>`))
			case 6:
				line = h6Reg.ReplaceAll(line, []byte(`<h6>$1</h6>`))
			}
		}
